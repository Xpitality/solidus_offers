...
task :test_app do
  ENV['LIB_NAME'] = 'solidus_offers'
  Rake::Task['common:test_app'].invoke
end

require 'solidus_offers'
namespace :solidus_offers do
  ...
  task :sync do
    ...
    Dir["#{locales_dir}/*.yml"].each do |filename|
      ...
      (comments, other) = SolidusOffers::Utils.read_file(filename, basename)
      ...
      SolidusOffers::Utils.write_file(filename, basename, comments, other, false)
    end
  end

  def translation_keys
    (dummy_comments, words) = SolidusOffers::Utils.read_file(File.dirname(__FILE__) + '/default/solidus_core.yml', 'en')
      words
  end
